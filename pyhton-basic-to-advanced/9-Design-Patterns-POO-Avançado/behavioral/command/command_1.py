...
class Light:  # Receiver - Luz inteligente
    ...
    def change_color(self, color: str) -> None:
        self.color = color
        print(
            f'Light {self.name} in {self.room_name} changed color to {self.color}')


class ICommand(ABC):  # ICommand - Interface de comando
    @abstractmethod
...
class RemoteController:  # Invoker - Controle inteligente
    def __init__(self) -> None:
        self._buttons: Dict[str, ICommand] = {}
        self._undo_buttons: List[Tuple[str, str]] = []
    ...
    def button_pressed(self, name: str) -> None:
        if name in self._buttons:
            ...
            self._undo_buttons.append((name, "execute"))

    def button_again(self, name: str) -> None:
        if name in self._buttons:
            self._buttons[name].undo()
            self._undo_buttons.append((name, "undo"))

    def global_undo(self) -> None:
        if not self._undo_buttons:
            print("Nothing to undo")
            return None
        button_name, action = self._undo_buttons[-1]
        ...
        self._undo_buttons.pop()


if __name__ == "__main__":
    bedroom_light = Light('Bedroom', 'Bedroom')
    ...
    remote.button_again('first_button')

    remote.button_pressed('second_button')
    remote.button_again('second_button')

    remote.button_pressed('third_button')
    # remote.button_again('third_button')

    remote.button_pressed('fourth_button')
    remote.button_again('fourth_button')
